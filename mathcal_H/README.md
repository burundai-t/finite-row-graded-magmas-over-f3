# Stage 6 Theorem Bundle

Compact theorem/certificate bundle for Stage 6, prepared on 2026-05-03 and
integrated into the PAB support repository on 2026-05-04.
...
the root `scripts/` and `tables/` corpus.
...
short witness sanity check embedded in `verify_stage6_bundle.py` and documented
in `stage6.md`.
...
This check is embedded in `verify_stage6_bundle.py` and has no separate raw
artifact directory.
...
The Stage 6 bundle is not a replacement for the root `scripts/` and `tables/`
corpus.  The root verifier checks the compact manuscript-facing finite tables;
this bundle audits the external global hidden-continuation theorem package.
