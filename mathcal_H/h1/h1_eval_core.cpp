...
struct RangeStats {
    ...
    std::uint64_t first_upper_index = 0;
    std::uint64_t first_lower_index = 0;
    std::string min_word;
    std::string max_word;
    std::string first_upper_word;
    std::string first_lower_word;
    Metrics min_metrics;
    Metrics max_metrics;
    Metrics first_upper_metrics;
    Metrics first_lower_metrics;
};
...
RangeStats evaluate_range(std::uint64_t start, std::uint64_t end, int upper_target, int lower_target) {
    ...
    for (std::uint64_t idx = start; idx < end; ++idx) {
        ...
        if (m.rawH > st.max_rawH) {
            ...
            st.max_count = 1;
            st.max_index = idx;
            st.max_word = word_to_string(x);
            st.max_metrics = m;
        } else if (m.rawH == st.max_rawH) {
            ++st.max_count;
        }
        if (m.rawH >= upper_target) {
            if (st.upper_violations == 0) {
                st.first_upper_index = idx;
                st.first_upper_word = word_to_string(x);
                st.first_upper_metrics = m;
            }
            ++st.upper_violations;
        }
        if (m.rawH <= lower_target) {
            if (st.lower_violations == 0) {
                st.first_lower_index = idx;
                st.first_lower_word = word_to_string(x);
                st.first_lower_metrics = m;
            }
            ++st.lower_violations;
        }
        increment_word(x);
...
std::string range_json(const RangeStats& st) {
    ...
    os << ",\"upper_violations\":" << st.upper_violations;
    os << ",\"lower_violations\":" << st.lower_violations;
    if (st.upper_violations > 0) {
        os << ",\"first_upper_index\":" << st.first_upper_index;
        os << ",\"first_upper_word\":\"" << st.first_upper_word << "\"";
        os << ",\"first_upper_metrics\":" << metrics_json(st.first_upper_metrics);
    }
    if (st.lower_violations > 0) {
        os << ",\"first_lower_index\":" << st.first_lower_index;
        os << ",\"first_lower_word\":\"" << st.first_lower_word << "\"";
        os << ",\"first_lower_metrics\":" << metrics_json(st.first_lower_metrics);
    }
    os << "}";
    return os.str();
}
...
void usage() {
    std::cerr
        << "Usage:\n"
        << "  h1_eval_core --word X21\n"
        << "  h1_eval_core --start N --end M [--upper-target 1218] [--lower-target -379]\n";
}
...
int main(int argc, char** argv) {
    try {
        ...
        int upper_target = 1218;
        int lower_target = -379;
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (arg == "--word") {
                ...
                have_range = true;
            } else if (arg == "--upper-target") {
                upper_target = parse_i32(need_value("--upper-target"));
            } else if (arg == "--lower-target") {
                lower_target = parse_i32(need_value("--lower-target"));
            } else if (arg == "--total") {
        ...
        RangeStats st = evaluate_range(start, end, upper_target, lower_target);
